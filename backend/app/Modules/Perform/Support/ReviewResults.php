 ...
 *   the group is "suppressed": no score, no reviewer count, no comments. Their reviewers are never named in an
...
final class ReviewResults
{
     ...
     * @return array{groups: array<string, array{reviewers: int|null, suppressed: bool}>, competencies: list<array{id: int, name: string, max: int, scores: array<string, float|null>, average: float|null}>, comments: list<array{type: string, competency_id: int, text: string, author: string|null}>}
     */
    public static function aggregate(array $types, array $competencies, array $rows, bool $anonymous): array
    {
        $reviewers = [];
        foreach ($rows as $row) {
        ...
        foreach ($types as $type) {
            ...
            $suppressed = $type->isProtected() && $count < self::MIN_REVIEWERS;
