...
Строка журнала пишется **после коммита** транзакции (`afterCommit`): откаченная правка не оставляет строки. Если
запись в журнал упала, основная операция не ломается — в лог уходит `audit.write_failed` (тип, id, действие, класс
ошибки, без значений).

### Маскирование и исключения — `Support\AuditPolicy`
- **Белый список по типу записи** (`AuditPolicy::SAFE_FIELDS`): значение видно только у явно безопасных полей —
  id связей, статусы, даты решений, тип/категория, флаги. **Всё остальное** — тексты документов, пути к файлам,
...
365 дней пачками по 1000 (каждая пачка — один ограниченный `DELETE … WHERE id IN (…)`), пока не кончатся строки или
не пройдут 20 секунд (вызов cron ограничен 60 с). Остаток дочищает следующий запуск; ответ:
`{deleted, batches, complete}`. Повторный запуск ничего лишнего не делает.

### Персональные данные (модуль Privacy)
`Privacy\AuditPersonalData` (секция `audit_log`) подключён к выгрузке и удалению данных человека:
- **выгрузка** — строки журнала о кандидате и его откликах или о сотруднике, уже замаскированные;
- **удаление** — подписей с именами в журнале нет (хранятся только id), поэтому строки остаются для истории, а
  значения вне текущего белого списка (например, записанные по старым правилам) заменяются на `***`.
  Сама анонимизация (новое ФИО «Видалений кандидат #…», очистка контактов) тоже пишется только как `***`.
