...
- Настройки как в Sintegrum: количество дней отпуска **на филиал** (`vacation_annual_days`, `sick_leave_annual_days`)
...
- `Support/WorkingDayCalculator::days(from, to, halfDay, holidays)` — пн–пт минус праздники; `start`/`end` снимают 0.5 с
  первого/последнего дня, если он рабочий; однодневный полудневный запрос = 0.5; обход не длиннее 366 дней.
- `Support/AccrualCalculator` — `yearly_upfront`: вся годовая норма в период `YYYY`, в год приёма — пропорционально
  месяцам (месяц приёма считается): приняли в октябре при 24 днях → 6; `monthly`: `annual/12` в каждый месяц работы
  (`YYYY-MM`); `expiring(balance, carryMax)` — сколько сгорает.
...
индекс; вставка внутри savepoint, чтобы на Postgres не ломать транзакцию). Пропущенные месяцы задним числом не
начисляются. Ответ: `{employees, accrued, expired}`. При создании сотрудника (`People\Events\EmployeeHired`) слушатель
...
`AccrualJobTest` (через `POST /api/ops/jobs/run`: год наперёд идемпотентно и пропорционально, помесячно, сгорание 1 января,
...
- Нет импорта балансов из Sintegrum и уведомлений (почта/Telegram) о новых запросах; нет «задонатить отпуск».
