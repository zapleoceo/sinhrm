...
## Как пользоваться
- Меню слева: «Огляд» (состояние системы); раздел «Адміністрування»: «Користувачі» и «Інтеграції» — только суперадмину, «Довідники» — суперадмину и админу.
...
- Маршруты (`app.routes.ts`): `/login` (гости, `guestGuard`), `/` → оболочка (`authGuard`) с детьми
  `''` — статус, `admin/users` и `admin/integrations` — `roleGuard('superadmin')`, `admin/directory` — `roleGuard('superadmin', 'admin')`. Экраны загружаются лениво.
