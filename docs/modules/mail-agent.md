...
уверен (≥ 85%), **правило создаётся само** (помечено «створено ШІ») и письма этого отправителя из очереди разбираются
...
`Ai/MailClassificationPrompt`, `mail_classify.v2`: адрес, тема ≤ 300, тело ≤ 1500 без цитат и подписи —
`Support/MailBodyCleaner`). Ответ применяет `Ai/MailClassificationAiHandler` (обычно в задаче `ai.poll`):
- `conf ≥ 0.85` (`AUTO_APPLY_CONFIDENCE`) → `MailAgentService::createAiRule()` — правило на точный адрес, `source = ai`,
...
Подсказки правил в очереди — `Support/SenderSuggester` (домены сайтов вакансий, `noreply/newsletter/…`).
