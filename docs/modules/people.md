...
|---|---|---|---|
| `GET /api/people` | любой активный | `q, branch_id, department_id, position_id, manager_id, status, perPage` 1..200 (строка `"20"` ок), `page` | пагинация, только уровень «справочник»; без `status` — все, кроме уволенных |
| `GET /api/people/{id}` | любой активный | — | профиль по уровням + `access` |
...
возвращает существующего. Создание сотрудника (вручную или наймом) шлёт событие `Events/EmployeeHired` — TimeOff сразу
...
записи не-админам, цикл руководителя, оргструктура и `mine`, `/me/employee`), `ChangeRequestApiTest` (белый список,
