...
у филиала ещё `city_id` и `city: {id, name} | null`. `city_id` принимается только для `branches` (иначе 422) и только id существующего **активного** города,
...
3. `GET {base_url}/cities/list`, `/branches/list`, `/departments/list`, `/jobs/list` (должности = `jobs`), заголовок
   `Authorization: Bearer <token>`, без редиректов, таймаут запроса 15 с (соединение 5 с), **все запросы — не дольше 45 с**
   (`TOTAL_SECONDS`). Функция Vercel живёт до 60 с (`maxDuration` в `backend/vercel.json`): 15 с запаса на загрузку
   Laravel, транзакцию и ответ, чтобы клиент получил понятный `sintegrum_timeout`, а не обрыв платформой.
...
соединения → 502 `sintegrum_unreachable`; неизвестный формат ответа → 502 `sintegrum_bad_response`; вышли за 45 с
