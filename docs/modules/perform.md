...
**Известное ограничение (вычитание между циклами):** если в двух циклах группа коллег одного сотрудника отличается
на одного человека (например, 3 и 4 оценщика), разница средних позволяет вычислить оценку добавленного коллеги.
Сравнения циклов в API нет, защиты от ручного вычитания тоже нет (аналогичная защита в Pulse — `SafeComparison`).
