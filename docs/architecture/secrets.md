...
В тестах используются только явно фейковые значения с префиксом `synthetic-`. `.gitleaks.toml` разрешает их
**только** в каталогах тестов; в остальном коде такие строки по-прежнему считаются утечкой.
